...
type ContactField = "full_name" | "first_name" | "last_name" | "account_name" | "title" | "department" | "email" | "phone" | "owner_name" | "last_activity_date";
type ImportRow = {
  rowNumber: number;
...
const aliases: Record<ContactField, string[]> = {
  ...
  account_name: ["company", "company name", "account", "account name", "client", "client name", "organisation", "organization"],
  ...
  last_activity_date: ["last activity", "last activity date", "last contacted", "last contacted date"],
};
...
function parseWorkbook(buffer: ArrayBuffer, accounts: Account[]): ImportRow[] {
  ...
  return source.map((row, index) => {
    const usedHeaders = new Set(Object.keys(aliases).map((field) => matchedHeader(row, field as ContactField)).filter(Boolean));
...
export function ContactImportDialog({ open, onOpenChange, contacts, accounts }: {
  open: boolean;
  ...
  const downloadTemplate = () => {
    const sheet = XLSX.utils.aoa_to_sheet([
      ["Full Name", "Company Name", "Designation", "Department", "Email", "Phone", "Owner Name", "Last Activity Date"],
      ["Ananya Sharma", "Example Company", "Head of L&D", "Human Resources", "ananya@example.com", "+91 98765 43210", "", "2026-08-18"],
    ]);
    sheet["!cols"] = [{ wch: 24 }, { wch: 28 }, { wch: 24 }, { wch: 20 }, { wch: 30 }, { wch: 20 }, { wch: 20 }, { wch: 20 }];
  ...
  const importRows = async () => {
    setImporting(true);
    try {
      const byEmail = new Map(contacts.filter((contact) => contact.email).map((contact) => [key(contact.email), contact]));
      ...
      for (const row of rows) {
        ...
        const duplicateKey = row.email ? `email:${key(row.email)}` : `contact:${key(`${row.first_name} ${row.last_name}`)}:${row.account_id ?? ""}`;
        if (seen.has(duplicateKey)) { skipped += 1; continue; }
        ...
        if (match) updated += 1; else inserted += 1;
      }
    ...
    <Dialog open={open} onOpenChange={(next) => { if (!next && !importing) reset(); onOpenChange(next); }}>
      <DialogContent className="max-h-[88vh] max-w-5xl overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Headers are detected automatically. Extra columns are saved with the contact. Company names must already exist under Corporate Clients.</DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          ...
          {!rows.length ? (
            <div className="rounded-lg border border-dashed border-border p-10 text-center">
              ...
              <p className="text-xs text-muted-foreground">Full Name or Last Name is required. Other headers are detected automatically.</p>
            </div>
          ) : (
            <>
              <div className="flex flex-wrap gap-2 text-sm">
                ...
                <span className="rounded-full bg-muted px-3 py-1 text-muted-foreground">Detected {headers.length} columns</span>
              </div>
              <div className="overflow-x-auto rounded-lg border border-border">
                <table className="w-full min-w-[920px] text-sm">
                  <thead className="bg-muted/60 text-left text-xs uppercase text-muted-foreground"><tr>
                    {["Row", "Contact", "Company", "Designation", "Email", "Phone", "Extra fields", "Status"].map((header) => <th key={header} className="px-3 py-2">{header}</th>)}
                  </tr></thead>
                  <tbody className="divide-y divide-border">{preview.map((row) => (
                    <tr key={row.rowNumber}>
                      <td className="px-3 py-2 text-muted-foreground">{row.rowNumber}</td>
                      <td className="px-3 py-2 font-medium">{[row.first_name, row.last_name].filter(Boolean).join(" ") || "—"}</td>
                      <td className="px-3 py-2">{row.account_name ?? "—"}</td><td className="px-3 py-2">{row.title ?? "—"}</td>
                      <td className="px-3 py-2">{row.email ?? "—"}</td><td className="px-3 py-2">{row.phone ?? "—"}</td>
        ...
        <DialogFooter>
          ...
          <Button type="button" disabled={!validCount || importing} onClick={() => void importRows()}>
            {importing ? <Loader2 className="size-4 animate-spin" /> : <Upload className="size-4" />}{importing ? "Importing…" : `Import ${validCount} contacts`}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
